...
exports.downloadTrainerTipsCSV = async (req, res) => {
  try {
    const userId = req.user._id;
    const tips = await TrainerTips.find({ userId }).sort({ createdAt: -1 });
    if (!tips.length) {
      return res.status(404).json({ error: "No tips found for this trainer." });
    }
    const fields = ["TipID", "Title", "Description", "CreatedAt"];
    ...
    tips.forEach(tip => {
      csvRows.push([
        ...
        tip.createdAt ? tip.createdAt.toISOString() : ""
      ].join(","));
    });
    const csv = csvRows.join('\r\n');
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename="trainer_tips_report.csv"');
