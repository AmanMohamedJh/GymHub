...
const getOwnerEquipment = async (req, res) => {
  try {
    const equipment = await Equipment.find({ userId: req.user._id }).populate(
      "gymId",
      "name"
    );
    res.status(200).json(equipment);
...
const getPublicGymEquipment = async (req, res) => {
  try {
    const { gymId } = req.params;
    const equipment = await Equipment.find({ gymId });
    res.status(200).json(equipment);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
const updateEquipment = async (req, res) => {
  try {
    ...
    if (!equipment) {
      return res
        .status(404)
        .json({ error: "Equipment not found or unauthorized" });
    }
...
const scheduleMaintenance = async (req, res) => {
  ...
  try {
    ...
    const addedMaintenance =
      equipment.maintenance[equipment.maintenance.length - 1];
...
const deleteMaintenance = async (req, res) => {
  ...
  try {
    ...
    equipment.lastMaintenanceDate = lastCompleted
...
const checkDueMaintenance = async (req, res) => {
  try {
    ...
    const dueMaintenance = updatedEquipment.flatMap((item) =>
      item.maintenance
        .filter((m) => m.status === "In Progress" && !m.notificationSent)
        .map((m) => ({
